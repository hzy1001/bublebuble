...
import bubble.game.compoment.Bubble;
	...
	public void run() {
		...
		while (true) {
			...
			for (int i = 0; i < bubbleList.size(); i++) {
				...
				if(bubble.getState() == 1) { //겅이 버블안에 있는 상태일때만 수행
					if ((Math.abs(player.getX() - bubble.getX()) < 10)
							&& Math.abs(player.getX() - bubble.getY()) > 0
							&& Math.abs(player.getY() - bubble.getY()) < 50) {
						System.out.println("적 사살");

						//new Thread(()->{ 
						//	bubble.clearBubbled();					//그림이 다시그려주는 순간 모든 객체가 지워졌다 다시생성되므로 clearBubbled 다음 벽충돌 체크시 에러발생되므로 별도의 스래드로 실행하도록 한다.						 
						//}).start();
						
						//메소드 안에서 쓰레드를 돌리는게 좋다.
						bubble.clearBubbled();
						break;
					}
				}
